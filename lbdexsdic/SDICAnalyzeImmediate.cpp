//===-- SDICAnalyzeImmediate.cpp - Analyze Immediates ---------------------===//
//
...
SDICAnalyzeImmediate::Inst::Inst(unsigned O, unsigned I) : Opc(O), ImmOpnd(I) {}
// Add I to the instruction sequences.
void SDICAnalyzeImmediate::AddInstr(InstSeqLs &SeqLs, const Inst &I) {
...
void SDICAnalyzeImmediate::GetInstSeqLsADDiu(uint64_t Imm, unsigned RemSize,
					     InstSeqLs &SeqLs) {
  GetInstSeqLs((Imm + 0x8000ULL) & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ADDiu, Imm & 0xffffULL));
}
void SDICAnalyzeImmediate::GetInstSeqLsORi(uint64_t Imm, unsigned RemSize,
					   InstSeqLs &SeqLs) {
  GetInstSeqLs(Imm & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ORi, Imm & 0xffffULL));
}
void SDICAnalyzeImmediate::GetInstSeqLsSHL(uint64_t Imm, unsigned RemSize,
					   InstSeqLs &SeqLs) {
  unsigned Shamt = countTrailingZeros(Imm);
  GetInstSeqLs(Imm >> Shamt, RemSize - Shamt, SeqLs);
  AddInstr(SeqLs, Inst(SHL, Shamt));
}
void SDICAnalyzeImmediate::GetInstSeqLs(uint64_t Imm, unsigned RemSize,
					InstSeqLs &SeqLs) {
...
// are replaced with
// LUi 0x444
void SDICAnalyzeImmediate::ReplaceADDiuSHLWithLUi(InstSeq &Seq) {
...
void SDICAnalyzeImmediate::GetShortestSeq(InstSeqLs &SeqLs, InstSeq &Insts) {
...
const SDICAnalyzeImmediate::InstSeq
&SDICAnalyzeImmediate::Analyze(uint64_t Imm, unsigned Size,
